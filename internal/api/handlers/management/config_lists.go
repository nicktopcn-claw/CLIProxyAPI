...
// embeddings-compatibility: []EmbeddingsCompatibility
func (h *Handler) GetEmbeddingsCompat(c *gin.Context) {
	c.JSON(200, gin.H{"embeddings-compatibility": normalizedEmbeddingsCompatibilityEntries(h.cfg.EmbeddingsCompatibility)})
}
func (h *Handler) PutEmbeddingsCompat(c *gin.Context) {
	data, err := c.GetRawData()
	if err != nil {
		c.JSON(400, gin.H{"error": "failed to read body"})
		return
	}
	var arr []config.EmbeddingsCompatibility
	if err = json.Unmarshal(data, &arr); err != nil {
		var obj struct {
			Items []config.EmbeddingsCompatibility `json:"items"`
		}
		if err2 := json.Unmarshal(data, &obj); err2 != nil || len(obj.Items) == 0 {
			c.JSON(400, gin.H{"error": "invalid body"})
			return
	...
	filtered := make([]config.EmbeddingsCompatibility, 0, len(arr))
	for i := range arr {
		normalizeEmbeddingsCompatibilityEntry(&arr[i])
		if strings.TrimSpace(arr[i].BaseURL) != "" {
			filtered = append(filtered, arr[i])
		}
	}
	h.cfg.EmbeddingsCompatibility = filtered
	h.cfg.SanitizeEmbeddingsCompatibility()
	h.persist(c)
}
func (h *Handler) PatchEmbeddingsCompat(c *gin.Context) {
	type embeddingsCompatPatch struct {
		Name          *string                              `json:"name"`
		Prefix        *string                              `json:"prefix"`
		BaseURL       *string                              `json:"base-url"`
		APIKeyEntries *[]config.OpenAICompatibilityAPIKey  `json:"api-key-entries"`
		Models        *[]config.EmbeddingsCompatibilityModel `json:"models"`
		Headers       *map[string]string                   `json:"headers"`
	}
	var body struct {
		Index int                   `json:"index"`
		Patch embeddingsCompatPatch `json:"patch"`
	}
	if err := c.ShouldBindJSON(&body); err != nil {
		c.JSON(400, gin.H{"error": "invalid body"})
		return
	}
	if body.Index < 0 || body.Index >= len(h.cfg.EmbeddingsCompatibility) {
		c.JSON(400, gin.H{"error": "index out of range"})
		return
	}
	entry := &h.cfg.EmbeddingsCompatibility[body.Index]
	if body.Patch.Name != nil {
		entry.Name = *body.Patch.Name
	}
	if body.Patch.Prefix != nil {
		entry.Prefix = *body.Patch.Prefix
	}
	if body.Patch.BaseURL != nil {
		entry.BaseURL = *body.Patch.BaseURL
	}
	if body.Patch.APIKeyEntries != nil {
		entry.APIKeyEntries = *body.Patch.APIKeyEntries
	}
	if body.Patch.Models != nil {
		entry.Models = *body.Patch.Models
	}
	if body.Patch.Headers != nil {
		entry.Headers = *body.Patch.Headers
	}
	h.cfg.SanitizeEmbeddingsCompatibility()
	h.persist(c)
}
func (h *Handler) DeleteEmbeddingsCompat(c *gin.Context) {
	if name := c.Query("name"); name != "" {
		out := make([]config.EmbeddingsCompatibility, 0, len(h.cfg.EmbeddingsCompatibility))
		for _, v := range h.cfg.EmbeddingsCompatibility {
			if v.Name != name {
				out = append(out, v)
			}
		}
		h.cfg.EmbeddingsCompatibility = out
		h.cfg.SanitizeEmbeddingsCompatibility()
		h.persist(c)
		return
	}
	if idxStr := c.Query("index"); idxStr != "" {
		var idx int
		_, err := fmt.Sscanf(idxStr, "%d", &idx)
		if err == nil && idx >= 0 && idx < len(h.cfg.EmbeddingsCompatibility) {
			h.cfg.EmbeddingsCompatibility = append(h.cfg.EmbeddingsCompatibility[:idx], h.cfg.EmbeddingsCompatibility[idx+1:]...)
			h.cfg.SanitizeEmbeddingsCompatibility()
			h.persist(c)
			return
		}
...
func normalizeEmbeddingsCompatibilityEntry(entry *config.EmbeddingsCompatibility) {
	if entry == nil {
		return
	}
	entry.BaseURL = strings.TrimSpace(entry.BaseURL)
...
func normalizedEmbeddingsCompatibilityEntries(entries []config.EmbeddingsCompatibility) []config.EmbeddingsCompatibility {
	if len(entries) == 0 {
		return nil
	}
	out := make([]config.EmbeddingsCompatibility, len(entries))
	for i := range entries {
		copyEntry := entries[i]
		if len(copyEntry.APIKeyEntries) > 0 {
			copyEntry.APIKeyEntries = append([]config.OpenAICompatibilityAPIKey(nil), copyEntry.APIKeyEntries...)
		}
		normalizeEmbeddingsCompatibilityEntry(&copyEntry)
		out[i] = copyEntry
	}
	return out
}
